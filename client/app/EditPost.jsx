...
const EditPost = () => {
  ...
  const { images, pickImage, setImages, removeImage, uploadImages } = useUploadMultipleImages();
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState({});
  // const { video, pickVideo, uploadVideo, setVideo } = useUploadVideo();
  const router = useRouter();

  const { onSubmit } = useAuthSubmit(`${process.env.EXPO_PUBLIC_API_URL}/posts/${id}`);
  ...
  useEffect(() => {
    const fetchPostData = async () => {
      try {
        ...
        setDescription(posts.data.description_content);
        setImages(posts.data.description_imgs);
        setDescriptionImages(posts.data.description_imgs);
      } catch (error) {
