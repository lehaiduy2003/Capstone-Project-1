...
const SignIn = () => {
    ...
    const {loading, error, onSubmit} = useAuthSubmit(`${process.env.EXPO_PUBLIC_API_URL}/auth/sign-in`);

    const handleSignIn = async () => {
        const data = await onSubmit({body: {email: email.current, password: password.current}});
        if (error) {
            console.error(error);
            return;
        }
