...
const PostList = forwardRef(({ products, onEndReached, isLoading, onScroll }, ref) => {
    ...
    <>
      {uniqueProducts.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>No post to display</Text>
        </View>
      ) : (
        <FlatList
          ref={ref}
          ...
          renderItem={({ item }) => (
            <View style={styles.itemContainer}>
              <TouchableOpacity onPress={() => router.push(`/postDetails?postId=${item._id}`)}>
