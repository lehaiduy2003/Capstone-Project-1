...
export default class ProductController extends BaseController {
  ...
  public async getProducts(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const products = await this.productService.read(parsedFilter);
  ...
  async updateProductById(req: Request, res: Response): Promise<void> {
    ...
    try {
      const productData = validateProduct(req.body);
      const id = new ObjectId(req.params.id);
      const isUpdated = await this.productService.updateProductById(id, productData);
  ...
  public async deleteProductById(req: Request, res: Response): Promise<void> {
    try {
      const id = new ObjectId(req.params.id);
      const isDeleted = await this.productService.deleteProduct(id); // Delete product
